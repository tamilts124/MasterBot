...
def get_process_history(process_id: str) -> str:
    ...
    Use this to audit the entire session context. By reviewing the chronological sequence of your [INPUT] and the resulting [OUTPUT], you can perfectly understand the current state of the program and whether it is currently waiting for input from you.
    """
    proc = ACTIVE_PROCESSES.get(process_id)
    if not proc:
        return f"[ERROR] Process {process_id} not found."
    
    return f"--- INTERACTION LOG FOR {process_id} ---\n{proc.get_full_log()}"
...
def send_to_process(process_id: str, input_text: str) -> str:
    ...
    proc = ACTIVE_PROCESSES.get(process_id)
    if not proc:
        return f"[ERROR] Process {process_id} not found or already terminated."
